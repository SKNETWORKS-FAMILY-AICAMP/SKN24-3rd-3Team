...
| 이름 | 한 줄 회고 |
| :--- | :--- |
| **김규호** | K: 데드라인 준수., 확실한 분업, 담당 업무에 대한 책임감, 팀원간 의견 조율, 
P: 파인튜닝 데이터 선정시 의견 대립, LLM에 대한 지식 부족으로 의견 조율시 많은 시간 소비
T: 나만의 개념 공부, 체계화된 기획 및 계획 설계  |
| **박수영** | 청킹하는데 많은 어려움을 겪었습니다. 대제목 소제목이 있다보니, 마크다운 스플릿터 등 다양한 방법을 써보았지만, 유사도가 나오지 않는 문제에 가장 많은 시간을 쏟았다. 이후에 유사도 성능을 끌어올리는데 모델별로 확연히 다르다는 점을 알게 되었고 다양한 모델의 비교를 하며 성능의 차이를 직접 확인해보았다.  |
| **박세현** |  | 다양한 기술을 이해해볼 수 있어서 좋았다. 현재 한창 발전 중인 기술이다 보니 장단점이 뚜렷하고 각 단점을 보완하는 기술들이 활발하게 나오고 있다고 느꼈다.  또한 어떤 기술을 선택할 때 여러 지표를 바탕으로 장단점을 파악하고 수치에 근거하여 결정하는 접근을 배울 수 있어서 좋았다.
| **이동민** | | 이번 프로젝트를 통해 RAG 시스템은 단순히 문서를 넣는 것만으로 완성되지 않고, 전처리·청킹·검색 구조 설계가 함께 중요하다는 점을 배웠다. 특히 단일 청크 방식의 한계를 확인한 뒤 Parent-Child 구조로 개선하면서 검색 정확도와 문맥 유지의 균형이 중요하다는 것을 느꼈다. 또한 BM25, ReRank, 파인튜닝을 적용해 보며 답변 품질을 높이기 위한 다양한 방법을 직접 경험할 수 있었다
