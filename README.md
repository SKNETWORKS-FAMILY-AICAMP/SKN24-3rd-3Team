...
현행법(산업안전보건법 제17조)상 **공사금액 50억 원 미만의 소규모 건설 현장**은 안전관리자 선임 의무가 면제되어, 사실상 현장소장(관리감독자)이 현장 통제와 안전 관리의 모든 책임을 짊어지는 구조입니다. 

하지만 산업안전보건법 시행규칙에 따른 **관리감독자의 정기 안전보건교육은 연간 16시간**에 불과합니다. 이는 고도의 전문성과 다양한 위험성평가 지식이 요구되는 현장 안전을 비전문가인 현장소장이 오롯이 감당하기에는 턱없이 부족한 시간입니다.

이를 보완하기 위해 외부 전문기관을 통한 **'건설재해예방기술지도'** 제도가 의무화되어 있으나, 이마저도 **15일에 1회(약 2주 간격)** 방문 지도에 그치고 있습니다. 매일 공정이 바뀌고 투입되는 인력과 장비가 달라져 새로운 위험 요인이 실시간으로 발생하는 건설 현장의 특성을 고려할 때, 2주 간격의 간헐적 점검으로는 급변하는 현장 상황에 즉각적이고 실효성 있는 조치를 취하는 것이 사실상 불가능한 실정입니다.
...
> **출처:** [국가법령사이트] [산업안전보건법 시행규칙](https://www.law.go.kr/LSW//lsBylInfoPLinkR.do?lsiSeq=271485&lsNm=%EC%82%B0%EC%97%85%EC%95%88%EC%A0%84%EB%B3%B4%EA%B1%B4%EB%B2%95+%EC%8B%9C%ED%96%89%EA%B7%9C%EC%B9%99&bylNo=0004&bylBrNo=00&bylCls=BE&bylEfYd=20260101&bylEfYdYn=Y)
...
- 임베딩 검색은 `ko-sroberta-multitask`를 중심으로 구성, `Cohere-Rerank`를 적용
- `GPT-4.1-mini`는 검색된 문서를 바탕으로 사용자에게 답변을 생성하는 데 사용
...
RAG 기반 챗봇의 답변 품질은 **'질문에 맞는 정확한 문서를 얼마나 잘 찾아오느냐(Retrieval Performance)'**에 달려 있습니다. 본 프로젝트에서는 전문적인 건설 안전 기술 지침(KOSHA Guide) 데이터를 정확히 검색하기 위해, 다양한 검색 방식을 활용하여 성능을 평가하였습니다.

### 10.1 RAG 검색 성능 최적화

#### 1) 성능 평가를 위한 데이터셋 생성
가장 먼저, 실제 건설 현장에서 발생할 수 있는 구체적인 상황을 가정하여 10가지의 **테스트 질문 세트(Test Queries)**를 직접 생성하였습니다. 이는 모두 PDF 기반의 질문들입니다. 

<p align="center">
  <img src="./assets/images/테스트질문.png" width="700" alt="구축된 10가지 테스트 질문 세트">
  <br>
  <em>[이미지 1] 성능 평가에 사용된 데이터셋 예시</em>
</p>
...
* **ReRank:** 검색된 결과의 순위를 LLM(위 프로젝트에서는 gpt-4.1-mini)을 통해 다시 매기는 방식
