...
**프로젝트 기간**: 2026.04.08 ~ 2026.04.09 (2일)
...
```
{"input": "후꾸루 제대로 깔았어?", "output": "한지 바닥에서 완전히 접착하지 않고 띄워 바른 밑 바탕 종이를 제대로 깔았습니까?"}
{"input": "네지 빠진 데 없지?", "output": "나사가 빠진 곳이 없습니까?"}
{"input": "이치부 치수 맞춰서 재봤어?", "output": "1부 치수를 맞추어 재어 보셨습니까?"}
{"input": "야끼 잘 됐나 확인해봐.", "output": "구움이 잘 되었는지 확인해 주시겠습니까?"}
{"input": "도테 단단하게 쌓았어?", "output": "둑을 단단히 쌓으셨습니까?"}
{"input": "이모노 가다 틀리지 않았지?", "output": "주형이 틀리지 않았습니까?"}
```
...
| 이름 | 한 줄 회고 |
| :--- | :--- |
| **김규호** | **K:** 데드라인 준수, 확실한 분업, 담당 업무에 대한 책임감, 팀원 간 의견 조율<br>**P:** 파인튜닝 데이터 선정 시 의견 대립, LLM에 대한 지식 부족으로 의견 조율에 많은 시간 소비<br>**T:** 개념 학습, 체계화된 기획 및 계획 설계 |
| **박수영** | 청킹 과정에서 많은 어려움을 겪었다. 대제목과 소제목 구조가 있어 마크다운 스플릿터 등 다양한 방법을 시도했지만, 원하는 유사도가 나오지 않는 문제가 가장 큰 과제였다. 이후 유사도 성능이 모델별로 확연히 다르다는 점을 알게 되었고, 여러 모델을 비교하며 성능 차이를 직접 확인할 수 있었다. |
| **박세현** | 다양한 기술을 이해해볼 수 있어서 좋았다. 현재 빠르게 발전 중인 분야인 만큼 장단점이 뚜렷했고, 각 단점을 보완하는 기술들도 활발히 등장하고 있음을 느꼈다. 또한 기술을 선택할 때 여러 지표를 바탕으로 장단점을 파악하고, 수치에 근거해 결정하는 접근을 배울 수 있어서 의미 있었다. |
| **이동민** | 이번 프로젝트를 통해 RAG 시스템은 단순히 문서를 넣는 것만으로 완성되지 않으며, 전처리·청킹·검색 구조 설계가 함께 중요하다는 점을 배웠다. 특히 단일 청크 방식의 한계를 확인한 뒤 Parent-Child 구조로 개선하면서 검색 정확도와 문맥 유지의 균형이 중요하다는 것을 느꼈다. 또한 BM25, ReRank, 파인튜닝을 적용해 보며 답변 품질을 높이기 위한 다양한 방법을 직접 경험할 수 있었다. |
| **최하진** | 파인튜닝을 단순히 답변 생성 전 단계로만 보던 관점에서 벗어나, 은어 번역을 위한 별도의 모델을 활용하는 방식으로 사고를 확장할 수 있었다. 한국어 특화 모델이라 하더라도 기대만큼의 성능이 나오지 않는 경우가 있어 적절한 모델을 찾는 과정이 가장 어려웠다. 또한 팀원들과의 용어 차이에서 오는 이해의 간극을 조율하며 커뮤니케이션 역량 역시 성장할 수 있었다. |
